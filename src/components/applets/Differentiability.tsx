  ...
  return (
    <JSXGraphBoard
      ...
      setup={(board: JXG.Board) => {
        ...
        // The Function Graph
        const curve = createFunctionGraph(
          board,
          f,
          [-10, 10],
          {
        ...
        // Tangent candidate line: L(x) = f(x0) + m(x-x0)
        const tangentFunc = (x: number) => getY0() + getM() * (x - getX0());

        // Highlight when m is close to the true derivative
        const isBestApprox = () => Math.abs(getM() - df(getX0())) < 0.2;

        // Linear function L(x) (candidate tangent)
        createFunctionGraph(
          board,
          tangentFunc,
          [-10, 10],
        ...
        // R(x) = f(x) - L(x) shown as a vertical segment at Q
        createSegment(
          board,
          [Q, [() => Q.X(), () => tangentFunc(Q.X())]],
          {
            dash: 2,
            name: "R(x)",
            withLabel: true,
