...
The applet always integrates on $[min⁡(a,b),max⁡(a,b)][min(a,b),max(a,b)]$, so the order of the two gliders does not matter.
...
and computes the rectangle contribution $\varphi(\xi_i)(t_{i+1}-t_i)$. 
This is valid because $\varphi$ is constant on each open subinterval, hence $\varphi(\xi_i)$ is 
the same for any choice $\xi_i\in(t_i,t_{i+1})$. 
