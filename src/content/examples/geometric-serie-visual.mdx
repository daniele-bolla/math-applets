---
title: "Geometric series: a visual proof (Mabry’s triangle)"
...
# Geometric series: a visual proof (Mabry’s triangle)
...
Paper reference (open access): https://arxiv.org/pdf/2006.05062

This applet reproduces a classical picture (attributed to Mabry) that gives a visual proof of
...
In the code, the midpoint construction is implemented by repeatedly computing midpoints and drawing the corresponding shaded triangle. Conceptually, one step looks like:
```ts
const TL = mid(T, L);
const TR = mid(T, R);
const LR = mid(L, R);
